...
func run(runner: SceneTree) -> void:
	...
	ui.show_feedback("Reward received.")
	await _frames()
	t.check(not ui.feedback.get_global_rect().intersects(ui.hud.compass.get_global_rect()) and not ui.feedback.get_global_rect().intersects(ui.hud.target.get_global_rect()), "R46 gameplay feedback has its own space below compass and target labels")
	await _target_bar()
	await _inventory_and_shop()
	await _dialogue_and_loot()
	await _pending_rewards_and_death()
	await _journal_map()
	var notice_state: Dictionary = session.snapshot()
	ui.handle_action(MireTypes.success({"ui_action": "notice", "title": "Road notice", "text": "The road is open."}))
	await _frames()
	t.check(game.modes.mode == &"readable" and ui.panels[&"readable"].find_children("*", "Label", true, false)[0].text == "The road is open." and session.snapshot() == notice_state, "R35 current world notices use the readable panel without acquiring story evidence")
