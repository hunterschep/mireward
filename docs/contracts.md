...
WorldObject.configure(source_id,kind,snapshot) creates inactive fixed loot/readable geometry from the detached candidate. activate/deactivate manage live subscriptions after world commit. apply_persistent_state renders only its record. Loot emits ui_action=loot withentity_id; readables emit readable withdocument_id; validfinalwritreview emitswrit. Actor/purse pairs share the canonicalspawnID, withtheactor retaining world.entities ownership. See handoffs/T08-world-objects.md.
